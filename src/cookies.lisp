...
(defun load-session-key ()
  (let ((raw (uiop:read-file-string (merge-pathnames (asdf:system-source-directory "aoc")
                                                     #P".env"))))
    (cdr (assoc :session-key (cl-json:decode-json-from-string raw)))))

(defparameter *session-cookie* (make-cookie
                        :name "session"
                        :value (load-session-key)))
